...
```www```-subdomains are deprecated and should be redirected to the corresponding www-free (sub)domain. ```www.nightfever.org``` -> ```nightfever.org```, ```www.koeln.nightfever.org``` -> ```koeln.nightfever.org```

##Internationalized Domain Name (IDN)
Subdomains may use non-latin characters.
Since IDNs are still facing some implementation problems, they should always redirect to a sanitized subdomain (```köln.nightever.org``` = ```xn--kln-sna.nightfever.org``` ->  ```koeln.nightfever.org```).
...
#Sites & Subdomains
Sites for specific nightfever projects have their own subdomain.
...
The language identifier is omitted for the default language of a requested page/site (this includes single-language pages).
...
Example: ```nightfever.org/mail``` -> ```mail.nightfever.org``` 
